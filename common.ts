...
import { readableStreamToString, OnePasswordPartnershipError, parseIfJson } from "./error";
        ...
        if (e instanceof AxiosError && e.isAxiosError) {
            try {
                const responseBody =
                    ...
                    : e.response?.data
                throw new OnePasswordPartnershipError(e, parseIfJson(responseBody))
            } catch (innerError) {
                if (innerError instanceof ReferenceError) {
                    ...
                    throw new OnePasswordPartnershipError(e, e.response?.data)
                }
                if (innerError instanceof OnePasswordPartnershipError) {
                    // Got "OnePasswordPartnershipError" from the above try block
                    throw innerError;
                }
